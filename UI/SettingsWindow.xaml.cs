    ...
    public partial class SettingsWindow : Window
    {
        ...
        private void InitializeValues()
        {
            ...
            if (BreakSpeedLimitToggle.IsOn)
            {
                ...
                MaxScrollSpeedSlider.Maximum = ConfigManager.Current.BreakSpeedLimitMax;
            }
            else
            {
                BreakSpeedLimitPanel.Visibility = Visibility.Collapsed;
                MaxScrollSpeedSlider.Maximum = 2000;
            }
        ...
        private void CheckHotkeyConflicts()
        {
            ...
            {
                WpfMessageBox.Show(
                    (string)FindResource("HotkeyConflictMessage"),
                    (string)FindResource("HotkeyConflictTitle"),
        ...
        private void DelayStartToggle_IsOnChanged(object sender, RoutedPropertyChangedEventArgs<bool> e)
        {
            if (e.NewValue)
            {
                ...
                DelaySliderPanel.BeginAnimation(OpacityProperty, anim);
            }
            else
            {
        ...
        private void BtnReadingModeListen_Click(object sender, RoutedEventArgs e)
        {
        ...
        private void StartReadingModeListening()
        {
            if (_mouseHook == null && _keyboardHook == null)
            {
                WpfMessageBox.Show("No input hooks available.", "Error", MessageBoxButton.OK, MessageBoxImage.Warning);
            ...
            if (_mouseHook != null)
            {
                _mouseHook.MouseEvent += OnMouseEventForReadingModeListening;
            }
        ...
        private void StopReadingModeListening()
        {
            _isReadingModeListening = false;
            BtnReadingModeListen.Content = FindResource("BtnListen") ?? "Listen";
            ReadingModeHotkeyInput.IsEnabled = true;
        ...
        private void OnKeyboardEventForReadingModeListening(object? sender, KeyboardEventArgs e)
        {
            if (e.Message != NativeMethods.WM_KEYDOWN && e.Message != NativeMethods.WM_SYSKEYDOWN)
                return;
        ...
        private void OnMouseEventForReadingModeListening(object? sender, Core.MouseEventArgs e)
        {
            string? keyName = null;
        ...
        private void BreakSpeedLimitToggle_IsOnChanged(object sender, RoutedPropertyChangedEventArgs<bool> e)
        {
            ...
            {
                BreakSpeedLimitPanel.Visibility = Visibility.Visible;
                MaxScrollSpeedSlider.Maximum = BreakSpeedMaxSlider.Value;
                var anim = new DoubleAnimation(0, 1, TimeSpan.FromMilliseconds(200)) { EasingFunction = new QuadraticEase { EasingMode = EasingMode.EaseOut } };
                BreakSpeedLimitPanel.BeginAnimation(OpacityProperty, anim);
            }
            else
            {
                BreakSpeedLimitPanel.Visibility = Visibility.Collapsed;
                MaxScrollSpeedSlider.Maximum = 2000;
                if (MaxScrollSpeedSlider.Value > 2000)
                    MaxScrollSpeedSlider.Value = 2000;
            }
            ConfigManager.Save();
        }
        ...
        {
            var result = WpfMessageBox.Show(
                (string)FindResource("ConfirmResetMessage"),
                (string)FindResource("ConfirmResetTitle"),
                MessageBoxButton.YesNo, MessageBoxImage.Warning);

            if (result != MessageBoxResult.Yes) return;

            var d = ConfigManager.Defaults;
            ConfigManager.Current.Friction = d.Friction;
            ConfigManager.Current.InertiaMultiplier = d.InertiaMultiplier;
            ConfigManager.Current.ResponseTime = d.ResponseTime;
            ...
            InertiaSlider.Value = d.InertiaMultiplier;
            ResponseTimeSlider.Value = d.ResponseTime * 1000;
